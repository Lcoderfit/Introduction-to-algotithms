...
4.GOPROXY: go下载第三方库所使用的代理(https://goproxy.cn,direct)
...
### 2.2 常量声明(iota 美\[aɪˈoʊtə\])
* 批量声明常量时，如果某一行没写，则默认与上一行一样
* iota：常量计数器，在同一个const块中，第一个出现的iota的值均为0
* 每新增一行常量声明，iota自增1
```text
// 批量声明常量时，如果某一行没写，则默认与上一行一样
const (
...
// 由于b和c在声明时均没有赋值，所以默认b=iota+1 c=iota+1
// 而由每增加一行常量声明，iota的值会自增1，所以b=1+1 c=2+1
const (
	a = iota+1
	b
	c
...
uint 在32位操作系统上为uint32, 64位操作系统上为uint64
...
a := &Student{id:1, name: "xm"}
fmt.Printf("%v\n", a) // &{1, xm}，直接打印值
fmt.Printf("%+v\n", a) // &{id:1, name:xm} 先打印出字段名，然后打印值
fmt.Printf("%#v\n", a) // &main.Student{id:1, name:xm} 先打印结构体名称，然后打印字段名，最后打印字段名称

fmt多个内置函数
```

### 2.5 字符串/字符/反引号/strings包
```text
GO语言中字符串必须用双引号进行包裹
...
ascii类型/英文类型使用byte（因为只占一个字节），中文字符使用rune(占多个字节)
...
## 三、for/switch/运算符 等语句
### 3.1 for循环四种用法（没有while）
```text
// age用完即销毁
if age:=1; age > 0 {
    fmt.Printf("%d", age)
...
### 3.2 switch的四种用法
```text
// 1.switch 常量, 则case语句中必须是一个或多个常量或常量表达式，不能是逻辑表达式，例如case n > 10这种是会报错的
// 因为n > 10的结果是bool类型，而n为整型，两种类型不匹配
// case中可以带有多个常量表示式 case 1,1+2,4; 但是需要注意一点，每个常量表达之间不能出现重复值，
// 否则会报错，例如 case 1, 2, 1（第一个和第三个常量表达式重复了），报错信息：duplicate case 1 in switch
// 所以只会匹配到一个case，不存在匹配到多个case的情况，且执行完匹配到的case之后默认break，如果每匹配到则会执行default后break
switch n {
case 1,2:
...
// 2.switch 赋值表达式;常量表达式  本质上与第一种式一样的，不过多了一个赋值的步骤
switch n := 4; n {
case 1,2,3,4:
}

// 3.switch后不加条件，相当于switch true
// case语句的值必须式一个bool类型的值，例如 case n > 1; case true; case flase
switch {
case n > 1:
case n  < 10:
}

// 4.fallthrough // 执行满足条件的case的下一个case
// 兼容C语言, 为了向前兼容，但是不鼓励用这个
switch {
case s == "a":
    fmt.Println("a")
    fallthrough 
...
a2 := [3]int{1, 2} // 则a2[2]会默认为0

3.根据初始值自动推断长度
a := [...]int{1,2,3}

4.根据索引初始化
a := [5]int{1,2} // a[2~4]默认为0
a := [5]int{0:1, 4:3} // a[1,2,3]均为0，a[0]为1，a[4]为3
...
切片的本质(是一个结构体类型，结构体中包含指向底层数组的指针，长度，容量三个字段)
注意：如果a是一个切片，则fmt.Printf("%p", a)所打印的地址是切片a所指向的底层数组元素的地址，fmt.Printf("%p", &a)打印的是切片结构体的地址
	b := [2]int{1, 2}
	a := b[1:]
    // 通过%p占位符打印的a的值与&b[1]的值相同，说明通过%p打印切片变量时，打印的其实是该切片执行的底层数组元素的地址
	fmt.Printf("%p, %p, %p, %p, %p\n", &a, a, b, &b, &b[1])
	a = append(a, 2, 3, 4, 5, 6, 7)
	// 扩容之后，通过%p打印的a的值与扩容之前不一致，说明扩容后开辟了一个新的底层数组，切片结构体中的指针字段指向了这个新开辟的数组
	// 通过%p打印的&a的值前后没有发生变化，因为&a表示的是切片结构体的地址，这个是不会随扩容发生变化的，变化的只是切片结构体中执行底层数组的指针
	fmt.Printf("%p, %p, %p, %p\n", &a, a, b, &b)
输出：
0xc000004480, 0xc00000a0a8, %!p([2]int=[1 2]), 0xc00000a0a0, 0xc00000a0a8
0xc000004480, 0xc00000e280, %!p([2]int=[1 2]), 0xc00000a0a0
...
// 长度是不可变的，所以事实上扩容是重新开辟了一个新的数组)，
// 则会导致切片a所指向的底层数组的地址发生变化
a := append(a, 10)
a := append(a, k...) // 将切片k拆开
```

### 4.3 Go切片如何删除(一种方式)/插入元素(三种方式)/清空/复制
```text
1.删除索引为index的元素
a := []int{1,2,3}
a = append(a[:index], a[index+1:]...)

2.在第i个位置插入,a表示原切片，b表示需要插入的元素或切片
// 方式一，双重append
a := []int{1,2,3}
a = append(a, append(b, a[i:]...)...)

// 方式二，copy+append,添加一个元素
a := []int{1, 2, 3}
a = append(a, 0) // 先扩容1个单位
copy(a[i+1:], a[i:]) // 将a[i:]向后移动一个单位
a[i] = x // 对第i个元素赋值

// 方式三，copy和append组合在中间位置插入多个元素
a := []int{1, 2, 3}
a = append(a, n) // 先扩容n个单位
copy(a[i+n:], a[i:]) // 将a[i:]向后移动n个单位
copy(a[i:], b) // 将b插入

3.清空
// a长度变为0，但是容量不变，容量与b相同，需要注意一点：添加元素时不能直接a[0]=xx,因为此时a的长度为0，需要用append添加元素
a := b[0:0]
...
### 4.4 Go扩容时的特点/预分配内容/扩容机制
```text
1.如果定义一个slice时，不定义大小而定义容量，则append效率会很快
a := make([]int, 0, 10)
a = append(a, 2)

2.如果append时超过了slice的容量，则会进行扩容，Go底层会重新创建一个新的数组，所以slice结构体中指向底层数组的指针会发生改变;
通过%p打印的切片的值其实是切片结构体中指向底层数组的指针的值，通过%p打印的&slice其实是切片结构体的指针
例如：
a := make([]int, 0, 1)
a = append(a, 1,2,3) // 此时超出容量，则切片a所指向的底层数组的指针值会发生变化(通过%p打印的a的值)

但是&a的值(实现slice的结构体的地址)不会发生变化
pa := &a
*pa = append(*pa, 1, 2, 3, 4) // pa的值是不会发生变化的

// 扩容机制
1.oldCap表示扩容前的容量大小，newCap表示扩容后的容量大小，cap表示append操作之后切片长度大小
2.首先判断经过append操作后的切片总长度是否大于两倍的为扩容前的切片容量大小
3.如果大于，则扩容后容量大小即为append操作后切片总长度
4.否则判断扩容前的切片长度是否小于1024，如果小于则扩容为两倍的原始容量大小
5.否则每次扩容增加原来的1/4容量大小，直到刚好大于append操作后的切片总长度
6.如果内存溢出，则扩容为所需要的容量大小
newCap := oldCap
doubleCap := 2 * newCap
if cap > doubleCap {
    newCap = Cap
...
### 4.5 copy/append/扩容举例
```text
a := []int{1,2,3}
b := a
copy(c, a) // 把内存拷贝一份(修改其中一个的值对另一个不影响)
a[0] = 100  // a: [100, 2, 3], b: [100, 2, 3]; c: [1,2,3]
...
### 4.6 取地址/引用（根据地址取值）/new/make/rand/sort.Ints/sort.Strings
```text
GO语言中不需要对地址进行操作，只需要记住两个符号:
1. &: 取地址
2. *: 根据地址取值
...
new和make均可用于申请内存，区别在于new返回类型的地址，make返回类型本身
new一般用于基本数据类型的内存申请
